...
** Defining annoying prefix keys

Here are all the prefix key definitions that might be necessary later
into this file. Using =general.el= somewhat avoids having to do this,
but use-package's :bind still requires this for some things.

#+begin_src emacs-lisp :tangle init.el
  (define-prefix-command 'search+)
  (global-set-key (kbd "M-s") 'search+)

  (define-prefix-command 'agenda-and-time)
  (global-set-key (kbd "C-c a") 'agenda-and-time)

  (define-prefix-command 'music)
  (global-set-key (kbd "C-c m") 'music)
  ...
  (global-set-key (kbd "C-c p") 'password-management)

  (define-prefix-command 'database)
  (global-set-key (kbd "C-c s") 'database)
...
#+begin_src emacs-lisp :tangle init.el
  (use-package diminish
    :straight t
    :diminish eldoc-mode global-visual-line-mode visual-line-mode org-indent-mode)
#+end_src
...
Easily open a file using sudo over =TRAMP=.
...
#+begin_src emacs-lisp :tangle init.el
  (general-define-key
   ...
   "C-c b t" 'aabm/generate-text-buffer)
#+end_src
...
#+begin_src emacs-lisp :tangle init.el
  (use-package dired
    ...
    :diminish all-the-icons-dired-mode dired-hide-dotfiles-mode dired-async-mode
    :bind
    (:map dired-mode-map
...
#+begin_src emacs-lisp :tangle init.el
  ...
  (general-define-key
   "C-c C-a" 'aabm/find-org-agenda-file)
#+end_src
...
#+begin_src emacs-lisp :tangle init.el
  (use-package calibredb
    ...
    :bind
    (("C-c s l" . calibredb)
     ("C-c s s" . calibredb-find-counsel)
     ("C-c s a" . calibredb-add)
...
#+begin_src emacs-lisp :tangle init.el
  (use-package mpdel
    :straight ivy-mpdel
    :bind
    (("C-c m m" . ivy-mpdel-list)
     ("C-c m l" . mpdel-playlist-open)
     ("C-c m b" . mpdel-browser-open)))
#+end_src
...
Using this configuration, the default start buffer is the *scratch*
buffer. I used to use a pretty elaborate [[*Dashboard][Dashboard]] setup, but decided
to scratch that for simplicity. Here we do some basic customizations
to the default scratch buffer, to make it a little nicer.
...
#+begin_src emacs-lisp :tangle init.el
  ...
  (set-face-attribute 'Info-quoted t
                      :inherit 'default)
...
start the Emacs server. We also define a keybinding for killing Emacs.
