...
Minions disables display of minor modes in the modeline. It's more
convenient than =diminish= because it disables this globally, not
per-package.
...
#+begin_src emacs-lisp :tangle init.el
  (use-package exwm
    ...
    (defun aabm/launch-matrix ()
      (interactive)
      (exwm-async-run "element-desktop"))

    (defun aabm/launch-primary-browser ()
      (interactive)
      (exwm-async-run "firefox"))

    (defun aabm/launch-secondary-browser ()
      (interactive)
    ...
    (setq exwm-input-global-keys
          `(
            ...
            ;; Bind "<s-end>" to "slock", a simple X display locker.
            ([<s-end>] . (lambda ()
                           (interactive)
                           (start-process "" nil "/usr/bin/slock")))
            ([?\s- ] . aabm/window-monocle)
            ([?\s-q] . delete-window)
            ([?\s-Q] . aabm/kill-buffer-and-window)
            ([?\s-f] . zoom-mode)
            ([?\s-o] . other-window)
            ...
            ([?\s-c] . aabm/launch-matrix)
            ([?\s-w] . aabm/launch-primary-browser)
            ([?\s-W] . aabm/launch-secondary-browser)
            ))
...
=transmission= is a simple frontend for the Transmission torrent
client. Transmission normally consists only of a simple daemon, and so
requires some manner of a frontend for convenience.
