...
way. So we disable those. Here we make use of the host system checks
we established earlier. I like having some of these GTK elements
visible on my desktop system, but not on others. Using a conditional,
we can enable and disable elements based on the host system.
