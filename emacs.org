...
A handy function for displaying a list of =info= nodes.
...
packages like =selectrum= and =embark=, which I (and many others in
the Emacs world recently) have come to appreciate. This one
unfortunately only supports manual completion, i.e. you have to type
=M-TAB= to pull up the completion buffer. I've started using =company=
for more serious programming, but I still keep corfu here as legacy
code.
...
The larger code completion package. This one allows customizing both
the backend and frontend, and does real time completion (as opposed to
corfu's on-demand completion). Integrates well with LSP clients.

Here we enable company for all programming modes, tell it to only
complete symbols after 3 characters have been typed, and make the
Space key not only cancel completion but also insert a space, which is
nicer when typing.
